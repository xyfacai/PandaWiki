...
const DragRecommend: FC<DragRecommendProps> = ({ data, columns = 2, refresh, onChange }) => {
  ...
  >
    <SortableContext items={data.map(item => item.id)} strategy={rectSortingStrategy}>
      <Box sx={{ display: 'grid', gridTemplateColumns: `repeat(${columns}, 1fr)`, gridGap: 4 }}>
        {data.map((item, idx) => <SortableItem
          key={idx}
          id={item.id}
          item={item}
          refresh={refresh}
