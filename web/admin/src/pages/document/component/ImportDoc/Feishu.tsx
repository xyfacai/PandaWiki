import { createNode, getFeishuCloudDocs, getFeishuDocDetail, getFeishuKBDocById, getFeishuKnowledgeBase, ImportDocByFeishuFormData, ImportDocListItem, ImportDocProps } from "@/api"
...
const FeishuImport = ({ open, refresh, onCancel, parentId = null }: ImportDocProps) => {
  ...
  const handleSelectedImportedData = async () => {
    ...
    const feishuItems = newItems.filter(it => (it.success === -1 && !it.id && selectIds.includes(it.url)))
        ...
        newQueue.push(async () => {
          try {
            ...
            setItems(prev => {
              ...
              return [...prev.slice(0, idx), { ...prev[idx], ...res[0], success: 1, id: nodeRes.id }, ...prev.slice(idx + 1)]
            })
          } catch (error) {
  ...
  const onSubmit = (data: ImportDocByFeishuFormData) => {
    setLoading(true)
    Promise.all([
      getFeishuKnowledgeBase(data),
      getFeishuCloudDocs(data)
    ]).then(([res1, res2]) => {
      if (res1.length > 0) {
        setList(res1.map(item => ({
  ...
  const renderItem = (item: ImportDocListItem & { space_id: string }, idx: number) => {
    return <Stack direction={'row'} alignItems={'center'} gap={1} key={item.url} sx={{
      px: 2,
      pl: '44px',
      py: 1,
      cursor: 'pointer',
  ...
  useEffect(() => {
    ...
    if (requestQueue.length === 0 && items.length > 0) {
      const allSuccess = items.every(item => item.success === 1 && item.id !== '-1' && item.id !== '')
      if (allSuccess) {
        setStep('done')
      }
    }
  ...
  >
    {step === 'pull' && (
      <Stack gap={1}>
        <Stack direction='row' alignItems={'center'} justifyContent={'space-between'} sx={{ fontSize: 14, lineHeight: '32px' }}>
          <Box sx={{ fontSize: 14, lineHeight: '32px' }}>
            App ID
            <Box component={'span'} sx={{ color: 'red', ml: 0.5 }}>*</Box>
          </Box>
          <Button size="small" component='a' href='https://pandawiki.docs.baizhi.cloud/node/01976929-0e76-77a9-aed9-842e60933464#%E9%80%9A%E8%BF%87%E9%A3%9E%E4%B9%A6%E6%96%87%E6%A1%A3%E5%AF%BC%E5%85%A5' target="_blank">使用方法</Button>
        </Stack>
        <Controller
          control={control}
          name="app_id"
    ...
    {step !== 'pull' && <Box sx={{
      ...
      {['import'].includes(step) && items.length > 0 && <Stack
        direction={'row'}
        alignItems={'center'}
        gap={1}
        component={'label'}
      ...
      <Stack>
        {list.map((item, idx) => <Box key={item.space_id}>
          <Stack component={'label'} htmlFor={`feishu-space-${item.space_id}-select-checkbox`} direction={'row'} alignItems={'center'} justifyContent={'space-between'} gap={1} sx={{
            px: 2,
            cursor: 'pointer',
            py: 1,
            borderBottom: '1px solid',
            ...
            <Stack direction={'row'} alignItems={'center'} gap={1} sx={{ fontSize: 14, flex: 1 }}>
              <Checkbox
                size="small"
                id={`feishu-space-${item.space_id}-select-checkbox`}
                sx={{ flexShrink: 0, p: 0, m: 0 }}
                disabled={items.filter(it => it.space_id === item.space_id).length === 0}
                checked={items.filter(it => it.space_id === item.space_id).length > 0 && items.filter(it => it.space_id === item.space_id).every(it => selectIds.includes(it.url))}
                onChange={() => {
                  const spaceItems = items.filter(it => it.space_id === item.space_id)
                  const spaceUrls = spaceItems.map(it => it.url)
                  const isAll = spaceItems.every(it => selectIds.includes(it.url))
                  if (isAll) {
                    setSelectIds(selectIds.filter(it => !spaceUrls.includes(it)))
                  } else {
                    setSelectIds([...new Set([...selectIds, ...spaceUrls])])
                  }
                }}
              />
              <Icon type='icon-wenjianjia' sx={{ fontSize: 14, flexShrink: 0, width: 20 }} />
              <Ellipsis sx={{ fontSize: 14, width: 0, flex: 1, fontWeight: 500 }}>{item.name}</Ellipsis>
            </Stack>
            {!item.done && <Button size="small" sx={{ p: 0, minWidth: 0, flexShrink: 0, height: 21 }} onClick={(event) => {
              event.stopPropagation()
              setList(prev => prev.map(it => it.space_id === item.space_id ? { ...it, loading: true } : it))
              getFeishuKBDocById({ space_id: item.space_id, ...getValues() }).then((res) => {
                setItems(prev => [...prev, ...(res || []).map(it => ({
                  title: it.title,
                  content: '',
                  url: it.url,
                  success: -1 as const,
                ...
                setList(prev => prev.map(it => it.space_id === item.space_id ? { ...it, loading: false } : it))
              })
            }}>拉取文档</Button>}
          </Stack>
          {item.loading && <Stack direction={'row'} alignItems={'center'} gap={1} sx={{
            px: 2,
            py: 1,
            borderBottom: '1px solid',
            borderColor: 'divider',
          ...
          {(items.filter(it => it.space_id === item.space_id).length > 0) ? items.filter(it => it.space_id === item.space_id).map((it, idx) => renderItem(it, idx))
            : item.done && <Box sx={{
              fontSize: 12,
              py: 1,
              textAlign: 'center',
        ...
        <Stack
          ...
          gap={1}
          component={'label'}
          htmlFor="feishu-cloud-select-checkbox" sx={{
            cursor: 'pointer',
            pl: 2,
            py: 1,
            borderBottom: '1px solid',
          ...
          <Stack direction={'row'} alignItems={'center'} gap={1} sx={{ fontSize: 14, flex: 1 }}>
            <Checkbox
              ...
              checked={items.filter(it => it.space_id === '').length > 0 && items.filter(it => it.space_id === '').every(it => selectIds.includes(it.url))}
              onChange={() => {
            ...
            />
            <Icon type='icon-yunpan' sx={{ fontSize: 20, flexShrink: 0, color: 'primary.main' }} />
            <Ellipsis sx={{ fontSize: 14, width: 0, flex: 1, fontWeight: 500 }}>飞书云盘</Ellipsis>
          </Stack>
        </Stack>
        {items.filter(it => it.space_id === '').length > 0 ? items.filter(it => it.space_id === '').map((it, idx) => renderItem(it, idx))
          : <Box sx={{
            fontSize: 12, py: 1, textAlign: 'center', color: 'text.auxiliary',
            borderBottom: '1px solid',
            borderColor: 'divider',
          }}>暂无数据</Box>}
      </Stack>
