...
const TypeCount = ({ tab }: { tab: ActiveTab }) => {
  ...
  const list = [
    { label: '访问次数', value: 'page_visit_count', color: '#021D70', bg: 'linear-gradient( 180deg, #D7EBFD 0%, #BEDDFD 100%)' },
    { label: '问答次数', value: 'conversation_count', color: '#021D70', bg: 'linear-gradient( 180deg, #D7EBFD 0%, #BEDDFD 100%)' },
    { label: '访问用户数', value: 'session_count', color: '#021D70', bg: 'linear-gradient( 180deg, #D7EBFD 0%, #BEDDFD 100%)' },
    { label: '来源 IP 数', value: 'ip_count', color: '#260A7A', bg: 'linear-gradient( 180deg, #F0DDFF 0%, #E6C8FF 100%)' }
  ]
  ...
  return <Stack direction={'row'} alignItems={'stretch'} gap={2}>
    {list.map(it => <Card key={it.value} sx={{
      color: it.color,
      background: it.bg,
      p: 2,
      flex: 1,
