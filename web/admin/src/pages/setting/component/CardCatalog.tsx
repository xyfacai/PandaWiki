...
import { Box, Button, FormControlLabel, Radio, RadioGroup, Slider, Stack } from "@mui/material"
...
const CardCatalog = ({ id, data, refresh }: CardCatalogProps) => {
  ...
  const onSubmit = (value: CatalogSetting) => {
    updateAppDetail({ id }, { settings: { ...data.settings, catalog_settings: value } }).then(() => {
      refresh(value)
  ...
  useEffect(() => {
    setValue('catalog_visible', data.settings?.catalog_settings?.catalog_visible || 1)
    setValue('catalog_folder', data.settings?.catalog_settings?.catalog_folder || 1)
    setValue('catalog_width', data.settings?.catalog_settings?.catalog_width ?? 260)
  }, [data])
    ...
    <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} sx={{ mx: 2 }}>
      ...
      <Controller
        ...
        render={({ field }) => <RadioGroup row {...field} onChange={(e) => {
          ...
          <FormControlLabel value={1} control={<Radio size='small' />} label={<Box sx={{ width: 100 }}>默认显示</Box>} />
          <FormControlLabel value={2} control={<Radio size='small' />} label={<Box sx={{ width: 100 }}>默认隐藏</Box>} />
        </RadioGroup>}
      />
    </Stack>
    <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} sx={{ mx: 2 }}>
