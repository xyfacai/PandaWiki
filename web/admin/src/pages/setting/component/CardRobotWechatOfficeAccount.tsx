import { AppDetail, getAppDetail, KnowledgeBaseListItem, updateAppDetail, WechatOfficeAccountSetting } from "@/api"
...
const CardRobotWechatOfficeAccount = ({ kb, url }: { kb: KnowledgeBaseListItem, url: string }) => {
  const [isEdit, setIsEdit] = useState(false)
  const [detail, setDetail] = useState<AppDetail | null>(null)
  const [isEnabled, setIsEnabled] = useState(false)

  const { control, handleSubmit, formState: { errors }, reset } = useForm<WechatOfficeAccountSetting>({
    defaultValues: {
  ...
  const getDetail = () => {
    getAppDetail({ kb_id: kb.id, type: 8 }).then(res => {
      ...
      reset({
        wechat_official_account_is_enabled: res.settings.wechat_official_account_is_enabled,
        wechat_official_account_app_id: res.settings.wechat_official_account_app_id,
        wechat_official_account_app_secret: res.settings.wechat_official_account_app_secret,
        wechat_official_account_token: res.settings.wechat_official_account_token,
        wechat_official_account_encodingaeskey: res.settings.wechat_official_account_encodingaeskey,
      })
    })
  }
  ...
  const onSubmit = (data: WechatOfficeAccountSetting) => {
    ...
    updateAppDetail({ id: detail.id }, {
      settings: {
        wechat_official_account_is_enabled: data.wechat_official_account_is_enabled,
        wechat_official_account_app_id: data.wechat_official_account_app_id,
        wechat_official_account_app_secret: data.wechat_official_account_app_secret,
        wechat_official_account_token: data.wechat_official_account_token,
        wechat_official_account_encodingaeskey: data.wechat_official_account_encodingaeskey,
      }
    }).then(() => {
  ...
  return <>
    ...
    <Stack gap={2} sx={{ m: 2 }}>
      <Stack direction={'row'} alignItems={'center'} gap={2}>
        <Box sx={{ width: 156, fontSize: 14, lineHeight: '32px' }}>微信公众号</Box>
        <Controller
          control={control}
        ...
        <Stack direction='row' gap={2} alignItems={'center'} justifyContent={'space-between'}>
          ...
          <Controller
            ...
            render={({ field }) => <TextField
              ...
              helperText={errors.wechat_official_account_app_secret?.message}
            />}
          />
        </Stack>
        <Stack direction='row' gap={2} alignItems={'center'} justifyContent={'space-between'}>
        ...
        <Stack direction='row' gap={2} alignItems={'center'} justifyContent={'space-between'}>
          ...
          <Controller
            ...
            render={({ field }) => <TextField
              ...
              helperText={errors.wechat_official_account_encodingaeskey?.message}
            />}
          />
        </Stack>
      </>}
